...
## Email (SendGrid SMTP) Setup

To enable email verification and notifications, configure SendGrid SMTP credentials in your `.env` file:
...
- `SMTP_HOST`: Always `smtp.sendgrid.net` for SendGrid
- `SMTP_PORT`: Usually `587` (TLS)
- `SMTP_USERNAME`: Use `apikey` (literally the word 'apikey') for SendGrid
- `SMTP_PASSWORD`: Your SendGrid API key
- `EMAIL_FROM`: The verified sender email in your SendGrid account

**Production requirement:**
- The backend will refuse to start in production if any of these are missing or use a placeholder value.
- In development, a warning will be logged if placeholders are present.

See [SendGrid docs](https://docs.sendgrid.com/for-developers/sending-email/integrating-with-the-smtp-api) for more details.
