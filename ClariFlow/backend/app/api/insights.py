...
async def generate_insight(query: InsightQuery):
    ...
    try:
        ...
        # Map backend fields to frontend contract
        mapped_response = {
            "id": response.id,
            "insight": response.answer,
            "confidence_score": response.confidence_score,
            "data_summary": response.supporting_data.get("summary") if response.supporting_data else None,
            "recommendations": response.recommendations,
            "visualization_data": response.visualization_data,
            "analysis_type": response.insight_type,
...
async def insights_health_check():
    ...
    try:
        ...
        import openai
        # Simple test call
        response = openai.OpenAI(api_key=insight_service.client.api_key).chat.completions.create(
            model="gpt-3.5-turbo",
        ...
        health_status = {
            ...
            "timestamp": openai.OpenAI(api_key=insight_service.client.api_key).chat.completions.create(
                model="gpt-3.5-turbo",
