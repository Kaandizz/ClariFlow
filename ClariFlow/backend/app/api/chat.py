...
async def chat(
    ...
    try:
        ...
        logger.error(f"Validation error in universal chat endpoint for user {current_user.email}: {str(e)}")
        raise HTTPException(
            status_code=400,
            detail=str(e)
        )
...
async def get_session_messages(
    ...
    try:
        ...
        logger.error(f"Validation error getting session messages for user {current_user.email}: {str(e)}")
        raise HTTPException(
            status_code=400,
            detail=str(e)
        )
...
async def create_session(
    ...
    """
    try:
        session_title = request.title if request.title is not None else "New Chat"
        session = chat_memory_service.create_session(db, session_title, user_id=str(current_user.id))
        if not getattr(session, 'created_at', None) or not getattr(session, 'updated_at', None):
            raise HTTPException(status_code=500, detail="Session missing created_at or updated_at timestamp")
