...
class CSRFMiddleware(BaseHTTPMiddleware):
    ...
    Requires a CSRF token in both a cookie and a custom header (X-CSRF-Token) for all state-changing requests.
    Safe-list certain endpoints (e.g., /auth/login, /auth/register, /auth/refresh, /crm/webhook) to avoid breaking auth and webhooks.
    """
    def __init__(self, app, safe_list=None):
        super().__init__(app)
        self.safe_list = safe_list or [
            '/auth/login', '/auth/register', '/auth/refresh', '/auth/google-login',
            '/crm/webhook', '/crm/webhook/', '/crm/webhook/hubspot', '/crm/webhook/salesforce'
        ]

    async def dispatch(self, request: Request, call_next):
        # Only enforce for state-changing methods
        if request.method in ["POST", "PUT", "PATCH", "DELETE"]:
            # Allow safe-listed endpoints
            path = request.url.path
            if not any(path.startswith(safe) for safe in self.safe_list):
                csrf_cookie = request.cookies.get("csrf_token")
                csrf_header = request.headers.get("X-CSRF-Token")
                if not csrf_cookie or not csrf_header or csrf_cookie != csrf_header:
                    logger.warning(f"CSRF validation failed for {path} from {get_client_ip(request)}")
                    return JSONResponse(
...
def setup_security_middleware(app):
    ...
    app.add_middleware(CSRFMiddleware)  # Add CSRF protection
